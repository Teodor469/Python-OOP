...
class BaseInfluencer(ABC):
    def __init__(self, username: str, followers: int, engagement_rate: float) -> None:
        ...
        self.campaigns_participated = [] #o	Keeps track of the campaigns in which the influencer has participated
    ...
    def display_campaigns_participated(self):
        ...
        campaign_info = [f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, Reached followers: {self.reached_followers(type(campaign).__name__)}" for campaign in self.campaigns_participated]
        campaigns_participated_str = '\n'.join(campaign_info)
